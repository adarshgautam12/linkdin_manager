let linkedinProfileUrl = "https://www.linkedin.com/in/";
...
async function sendMessageToActiveTab(msg, sendResponse) {
  try {
    ...
    setTimeout(async () => {
      const res = await chrome.tabs.sendMessage(gptTabId, msg);

      if (res) {
        chrome.tabs.get(linkedinTabId, function (tab) {
...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  ...
  if (msg.type == "data_save") {
    ...
    if (!gptTabId) {
      ...
      chrome.tabs.get(gptTabId).catch((e) =>
        chrome.tabs.create({ url: `https://chat.openai.com`,active: false }, (tab) => {
          gptTabId = tab.id;
        })
      );
